...
class ListUserAPIsView(APIView):
    """
    View for listing user-specific APIs.
    """
...
class CreateAPIView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            return JsonResponse(
                {"message": "API created successfully", "api": serializer.data}, status=status.HTTP_200_OK
            )
        except IntegrityError as e:
            # Handle the IntegrityError (duplicate endpoint)
            return JsonResponse({"error": "API endpoint already exists"}, status=400)


class APIViewWithData(APIView):
    """
    View for accessing API data with authentication.
    """
