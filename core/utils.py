...
def read_contents_from_csv_or_xlsx_file(file_path):
    ...
    try:
        if file_path.endswith(".xlsx") or file_path.endswith(".xls"):
            content = pd.read_excel(file_path, header=0, nrows=21).head(2) if file_path else dataframe
        else:
            content = pd.read_csv(file_path, header=0, nrows=21).head(2) if file_path else dataframe
