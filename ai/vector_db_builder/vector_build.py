...
import logging
from AI.open_ai_utils import get_embeddings
from AI.vector_db_builder.load_audio_and_video import LoadAudioAndVideo
from AI.utils import build_pdf, generate_response, download_file, resolve_file_path, split_documents
...
class VectorDBBuilder:
    ...
    @shared_task
    def create_vector_db(self, resource_file, chunk_size=1000, chunk_overlap=200):
        # resource = ResourceFile(resource)
        status = "failed"
        resource_id = resource_file.get('id')
        try:
            documents, status = self.load_documents(
