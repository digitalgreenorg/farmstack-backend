...
class  SupportTicketTestCaseForViews(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.support_resolution_url = reverse("support_tickets_resolutions-list")
        
        self.user_role = UserRole.objects.create(
            id="1",
            role_name="datahub_admin"
        )
        self_user_role_participant = UserRole.objects.create(id=3, role_name="datahub_participant_root")
        ...
        self.create_resolution_one = Resolution.objects.create(
            ticket= self.create_support_ticket_one,
            user_map= self.user_map,
            resolution_text= "resolution in self"
        )
        ...
        self.create_support_ticket_two=SupportTicketV2.objects.create(
            ...
            status="closed",
            user_map= UserOrganizationMap.objects.create(user_id=self.user_participant.id, organization_id=self.creating_org_participant.id)
        )

        refresh = RefreshToken.for_user(self.user_participant)
        refresh["org_id"] = str(self.user_map.organization_id) if self.user_map else None # type: ignore
        ...
        refresh.access_token["org_id"] = str(self.user_map.organization_id) if self.user_map else None # type: ignore
        refresh.access_token["map_id"] = str(self.user_map.id) if self.user_map else None
        refresh.access_token["role"] = str(self.user_participant.role_id)
        refresh.access_token["onboarded_by"] = str(self.user_participant.on_boarded_by_id)
    ...
    def test_suppport_ticket_retrieve_valid(self):
        support_id = self.create_support_ticket_one.id
        response = self.client.get(self.support_ticket_url+f"{support_id}/", secure=True)
        data = response.json()
    ...
    def test_suppport_ticket_retrieve_invalid(self):
        ...
        response = self.client.get(self.support_ticket_url+f"{random_uuid}/", secure=True)
        data = response.json()
    ...
    def test_suppport_ticket_update_valid(self):
        ...
        response = self.client.put(self.support_ticket_url+f"{support_id}/", json.dumps(support_valid_data), content_type='application/json', secure=True)
        data = response.json()
    ...
    def test_suppport_ticket_update_invalid(self):
        ...
        support_invalid_data = {
            'ticket_title':'Support ticket 1 updated'
        }
        response = self.client.put(self.support_ticket_url+f"{support_id}/", json.dumps(support_invalid_data), content_type='application/json', secure=True)
    ...
    def test_suppport_resolution_create_invalid_text(self):
        ...
        response = self.client.post(self.support_resolution_url, json.dumps(resolution_invalid_data), content_type="application/json", secure=True)
        data = response.json()
        # print("***test_suppport_resolution_create_invalid_text data***", data)
        # print("***test_suppport_resolution_create_invalid_text status_code***", response.status_code)
        assert response.status_code == 400
        ...
        data = response.json()
        assert response.status_code == 400
        assert data['resolution_text'] == ['Ensure this field has no more than 1000 characters.']
    ...
    def test_suppport_resolution_update_valid_text(self):
        ...
        response = self.client.put(self.support_resolution_url+f"{resolution_id}/", resolution_valid_data, content_type='application/json', secure=True)
        data = response.json()
    ...
    def test_suppport_resolution_update_invalid_text(self):
        ...
        response = self.client.put(self.support_resolution_url+f"{resolution_id}/", resolution_invalid_data, content_type='application/json', secure=True)
        data = response.json()
