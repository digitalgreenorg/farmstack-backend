...
from participant.models import Datasets, Organization, UserOrganizationMap, UsagePolicy, ResourceFile, \
    LangchainPgEmbedding, Messages, DatasetV2, DatahubDocuments, DatasetSubCategoryMap, Category, SubCategory, \
    DatasetV2File, StandardisationTemplate, Policy, ResourceUsagePolicy, LangchainPgCollection, Resource, \
...
from utils.file_operations import move_directory, create_directory
from utils.string_functions import check_special_chars
from utils.validators import validate_file_size, validate_document_type, validate_image_type, validate_dataset_size, \
...
# DATAHUB SERIALIZERS START FROM HERE >


class OrganizationSerializerExhaustive(serializers.ModelSerializer):
...
class ResourceFileSerializer(serializers.ModelSerializer):
    ...
    def get_collections(self, obj):
        ...
        # import pdb; pdb.set_trace()
        if collection:
            embeddings = LangchainPgEmbedding.objects.filter(collection_id=collection.uuid).values("embedding",
                                                                                                   "document")
        ...
        try:
            ...
            for resource_file in resource_files_data:
                ...
                if resource_file.get("type") == "api":
                    with open(resource_file.get("file").replace("/media/", ''),
                              "rb") as outfile:  # Open the file in binary read mode
                        # Wrap the file content using Django's ContentFile
                        django_file = ContentFile(outfile.read(),
                                                  name=f"{resource_file.get('file_name', 'file')}.json")  # You can give it any name you prefer
                    ...
                    serializer.save()
                    LOGGER.info(
                        f"Embeding creation started for url: {resource_file.get('url')} or file: {resource_file.get('url')}")
                    VectorDBBuilder.create_vector_db.delay(serializer.data)
